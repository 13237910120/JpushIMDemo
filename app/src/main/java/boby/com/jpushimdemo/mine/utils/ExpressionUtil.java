...
import boby.com.jpushimdemo.IMUISampleApplication;
...
public class  ExpressionUtil {
	...
	public static void dealExpression(Context context, SpannableString spannableString, Pattern patten, int start, Hashtable<Integer, GifDrawalbe> cache, Vector<GifDrawalbe> drawables) throws Exception {
		...
		while (matcher.find()) {
			...
			Log.e("key = " , IMUISampleApplication.getInstance().getGifMap().get(key));
			Field field = R.drawable.class.getDeclaredField(IMUISampleApplication.getInstance().getGifMap().get(key));
